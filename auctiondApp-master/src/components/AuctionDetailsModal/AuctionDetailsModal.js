...
const AuctionDetailsModal = ({ auction, onClose, web3, account }) => {
  ...
  const loadAuctionDetails = async () => {
    if (web3 && auction) {
      ...
      const buyNowPrice = await auctionContract.methods.price().call();
  ...
  const buyNow = async () => {
    ...
    try {
      const auctionContract = new web3.eth.Contract(AuctionABI.abi, auction);
      const buyNowPriceWei = web3.utils.toWei(buyNowPrice, 'ether');
  
      console.log("Bid Amount (Ether):", buyNowPrice);
      console.log("Bid Amount (Wei):", buyNowPriceWei);

      const transactionParameters = {
        to: auction, 
        from: account, 
        value: web3.utils.toHex(buyNowPriceWei),
        data: auctionContract.methods.bid().encodeABI()
      };
  ...
  return (
    <div className="auction-details-modal">
      ...
      {timeLeft !== "Auction ended" && (
        ...
        <div className="bid-section">
          {/* <input type="number" className="bid-input" value={bidAmount} onChange={(e) => setBidAmount(e.target.value)} placeholder="Enter price to confirm purchase" /> */}
          
          <p className="auction-info">Buy now price:  {buyNowPrice ? web3.utils.fromWei(buyNowPrice, 'ether') : '0'} ETH</p>
          {timeLeft !== "Auction ended" && (
          <button className="bid-button" onClick={buyNow}>Buy now</button>
          )}
        </div>
